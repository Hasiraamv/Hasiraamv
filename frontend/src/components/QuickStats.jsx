      ...
      {STATS.map((stat) => (
        ...
        >
          ...
          <div className="flex flex-col gap-0.5">
            <span className="truncate text-[15px] font-bold leading-none tracking-[-0.02em] text-ink">
              {stat.value}
            </span>
            <span className="text-[10px] font-medium leading-tight text-ink/45">{stat.sub}</span>
          </div>
          <span className="text-[10px] font-medium uppercase tracking-[0.08em] text-ink/40">{stat.label}</span>
        </motion.div>
      ))}
    </div>
  );
